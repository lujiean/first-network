...
instantiateChaincode() {
  ...
  if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
    ...
    peer chaincode instantiate -o orderer.example.com:7050 -C $CHANNEL_NAME -n mycc -l ${LANGUAGE} -v ${VERSION} -c '{"Args":["init","user1","MALE","26","PHONENO697","2018-10-20"]}' -P "AND ('Org1MSP.peer')" >&log.txt
    res=$?
    set +x
  else
    set -x
    ...
    peer chaincode instantiate -o orderer.example.com:7050 --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA -C $CHANNEL_NAME -n mycc -l ${LANGUAGE} -v 1.0 -c '{"Args":["init","user1","MALE","26","PHONENO697","2018-10-20"]}' -P "AND ('Org1MSP.peer')" >&log.txt
    res=$?
    set +x
  fi
  cat log.txt
